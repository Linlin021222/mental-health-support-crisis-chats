"""Strict Qwen2.5-3B QLoRA capacity experiment for Task 1 (V47).

The 2024 BigData Cup showed that fine-tuned instruction models can add a
meaningfully different decision boundary.  This stage-one gate uses one fixed
epoch and a predeclared 20% blend on the untouched outer user fold.  It does
not alter production unless a later full OOF experiment confirms the gain.
"""
...
OUTPUT = config.OUTPUT_DIR / "task1_qwen_lora_v47"
RESULTS = OUTPUT / "results.json"
ADAPTER = OUTPUT / "strict_adapter"
TRAINING_VERSION = "task1-qwen25-3b-qlora-v47"
...
PREFIX = (
    "Classify this Reddit post into exactly one suicide-risk level. "
    "Indicator: no explicit suicide mention. "
    "Ideation: explicit wish to die or suicidal thought, but no plan. "
    "Behavior: suicidal expression plus a plan, method, means, or self-harm behavior. "
    "Attempt: a past or recent suicide attempt is explicitly described.\nPost:\n"
)
...
class PromptDataset(Dataset):
    def __init__(self, texts, labels, tokenizer):
        ...
        suffix = tokenizer.encode("\nRisk level:", add_special_tokens=False)
        available = MAX_LENGTH - len(prefix) - len(suffix) - 1
        first = available // 2
        self.rows = []
        for text, label in tqdm(zip(texts, labels), total=len(texts),
                                desc="V47 prompt tokenization"):
...
def _model_and_tokenizer():
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        MODEL_NAME, num_labels=4, quantization_config=quantization,
...
@torch.no_grad()
def _infer(model, dataset, indices, description):
    model.eval(); rows = []
    for batch in tqdm(_loader(dataset, indices, False), desc=description):
        with torch.autocast(device_type="cuda", enabled=True):
            logits = model(input_ids=batch["input_ids"].cuda(non_blocking=True),
                           attention_mask=batch["attention_mask"].cuda(
                               non_blocking=True)).logits
        rows.append(torch.softmax(logits.float(), -1).cpu().numpy())
    return np.vstack(rows)


def _train(model, dataset, indices, labels):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for step, batch in enumerate(tqdm(loader, desc=f"V47 QLoRA epoch {epoch}"), 1):
            ...
            with torch.autocast(device_type="cuda", enabled=True):
                ...
                loss = torch.nn.functional.cross_entropy(
                    logits.float(), target, weight=weights,
                    label_smoothing=.03,
                ) / ACCUMULATION
        ...
        print(f"V47 epoch={epoch} loss={history[-1]['train_loss']:.5f}", flush=True)
    return history


def main():
    if not torch.cuda.is_available():
        raise RuntimeError("V47 requires CUDA")
    OUTPUT.mkdir(parents=True, exist_ok=True)
    seed_everything(config.SEED + 4747)
    ...
    history = _train(model, dataset, train_idx, labels)
    qwen_probability = _infer(model, dataset, valid_idx, "V47 untouched outer users")
    ...
    baseline = _risk_probability(texts, transformer, qwen_probability,
                                 lexical, v36, 0.0)
    candidate = _risk_probability(texts, transformer, qwen_probability,
                                  lexical, v36, PREDECLARED_WEIGHT)
    ...
    standalone_prediction = qwen_probability.argmax(1)
    standalone = {"risk_f1": float(f1_score(
        labels[valid_idx], standalone_prediction,
        average="weighted", zero_division=0))}
    optimistic = []
    for weight in (0., .1, .2, .3, .4, .5):
        prediction = _risk_probability(texts, transformer, qwen_probability,
                                       lexical, v36, weight)
        evidence = _v18_evidence(outer_records, seed2, prediction,
                                 evidence_parameters)
        phrase = np.asarray([_post_phrase_f1(x, y)
                             for x, y in zip(evidence, gold)])
        optimistic.append({"weight": weight, **metrics(prediction, phrase)})

    unique = np.unique(groups[valid_idx]); rng = np.random.default_rng(config.SEED + 4747)
    deltas = []
    for _ in range(4000):
    ...
    payload = {"training_version": TRAINING_VERSION, "model": MODEL_NAME,
        "method": {"quantization": "NF4 4-bit", "lora_rank": 8,
            "targets": ["q_proj", "v_proj"], "max_length": MAX_LENGTH,
            "epochs": EPOCHS, "predeclared_weight": PREDECLARED_WEIGHT},
        ...
        "optimistic_weight_diagnostic_not_for_adoption": optimistic,
